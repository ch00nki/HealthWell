...
from services.Ollama_medGemma_service import MedGemma4BService
import logging
...
async def diagnose(request: DiagnosisRequest, user = Depends(verify_token)):
    try:
        service = MedGemma4BService()
        logger.debug(f"Received symptoms: {request.symptoms}")
        logger.debug("Calling MedGemma4BService.get_diagnosis")
        diagnosis = await service.get_diagnosis(request.symptoms)
